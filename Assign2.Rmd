...
anova(m1, m0)
```
Model 1 fits significantly better than model 0 as indicated by $χ^2(1) = 231.97$, $p<.001$. This means that there are differences in intercept between children ($\sigma_{u0}^2$ > 0) and accounting for them by adding the random intercept term improves the model. 

*This means that the individula-level adjustment to the intercept $u_{0i}$ to the overall intercept $π_0i$ is a significant addition to the model.*

## What is the intraclass correlation?
Based on the ICC formula, we can compute and get:
...
Intraclass correlation is 0.48. We can conclude that the proportion of the total unexplained variance accounted by the grouping structure (child level) is quite high.

## What is your conclusion regarding the overall question regarding the necessity of performing a multilevel analysis?

We have shown that there is a significant difference between model 0 and model 1.  Therefore, it could be concluded that the intercept (mean) of *antisocial behavior* varies on the individual-level. Additionally, the ICC ($ρ_{school} = 0.48$) also signifies that there indeed are important differences in *antisocial behavior* on the individual-level, and 48% of the total variance is explained by it.

Therefore, we should adopt multilevel analysis.
