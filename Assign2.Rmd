...
2) there is a linear relationship between the level-2 predictors and the aggregated outcome, that is between *antisocial behavior* and *cognitive stimulation* and *antisocial behavior* and *mother’s age*.
...
The plot of the relation between *time* and *antisocial behavior* reveals that the quadratic trend line is practically the same as the linear trend line. It shows that the explained variance is almost the same for these two trends and the relation appears to be approximately linear.

As we can see from the *Figure 2*, the linear and quadratic lines for the relation between the *reading recognition skills* and *antisocial behavior* are now slightly different. The quadratic trend seems to fit somewhat better (and explain more variance in the outcome) than the linear one, albeit to a rather small degree. The slope of the linear trend line seems to be close to 0, indicating that there is no linear relation between these two variables. There appears to be neither linear nor quadratic relation between the two variables, but we will include *read* in the analysis to further evaluate this.

*Figures * show the relations between the first level predictors and the outcome for a subset of children. The relation between *time* and *antisocial behavior* slightly varies between the children. The relation between *reading recognition skills* and *antisocial behavior* is quite similar for different children in this subset and the slopes seem to be close to 0. 
...
p2 <- dat_long %>% 
  ...
  ggtitle("Relation between cognitive stimulation and aggregted Behavior Problems Index")+
  theme(plot.title = element_text(size = 10, hjust=0.5))
p2
```

*Figure * reveals that both linear and quadratic trend lines for the relationship between *cognitive stimulation* and *antisocial behavior* are again similar to each other; adding the quadratic trend to the model would not add additional information in predicting the outcome. We conclude that the relation seems to be approximately linear. 
...
Intraclass correlation equals 0.48. We see that almost a half of the total unexplained variance represents the individual-level differences, which is quite high.
...
- $\sigma_{u0}^2 > 0$, $ICC=.48$: a large part of variation in the outcome is on the child-level and accounting for this variation in the *antisocial behavior* across children by adding the random intercept term significantly improves the fit of the model;
...
The intercept, $b_{0} = 1.55, t(400) = 13.872, p < .001$, can be interpreted as the expected Behavior Problems Index at the first occasion. The coefficient for *time* is significant, $b_{time}=0.18, t(663) = 4.513, p <.001$. It means that with each one point increase in time, that is with each measurement occasion, the expected *antisocial behavior* score increases by 0.18.
...
Comparing this model with the model without the random slope, we see that the former fits the data significantly better, $χ^2(2) = 26.56$, $p<.001$, and its AIC is lower, $AIC_{m5a}=3293.3, AIC_{m4b}=3315.8$. This shows that adding the random slope term and the covariance between the slope and the intercept improves the model.
...
We can conclude that the relation between *antisocial behavior* and *time* varies across children and we should account for that by allowing the random slope for *time*. *Read* does not have a varying slope and including it in the model, together with the other parameters, does not improve the fit of the model. 
...
Comparing this model with the model with both interactions (m6a), we see that the latter does not fit the data better $χ^2(2) = 0.3$, $p>.05$. This is further confirmed by AIC values, $AIC_{m6b}=3288.4$, than of the model with both interactions, $AIC_{m6a}=3292.1$. 

Furthermore, we obtained a better-fitting model compared to the model without the slope predictors, m5a, $χ^2(1) = 6.88$, $p<.05$.

**Overall conclusion**

We conclude that adding the cross-level interaction between *cognitive stimulation* and *time* improves the model. *Cognitive stimulation* can explain a part of the variation in the trends in *antisocial behavior* over time across children. Adding *mother's age* as a predictor of slope for *time* does not improve the model and this variable does not moderate the relation between *time* and *antisocial behavior*. 
...
$$π_{0i} = β_{00} + β_{01}homecog + u_{0i}$$

$$π_{1i} = β_{10} + β_{11}homecog + u_{1i}$$

Mixed Model Equation :

$$Anti_{ti} = β_{00} + β_{01}homecog + β_{10}T_{ti} + β_{11}homecog*time + u_{0i} + u_{1i}T_{ti} + e_{ti}$$
...
# slope
qqnorm(ranef(m6b)$id[,2], main = "Q-Q plot for the slope of time variable")
qqline(ranef(m6b)$id[,2])
```

- First level residuals are approximately normally distributed with some outliers and minor deviations on left bottom and upper right corner of the plot.
- The residuals for the intercept and slope of *time* also have some outliers on each side and the middle part of the plot does not follow the line as well as the residuals on the first level. But we can conclude that the deviations are not too extreme and they are approximately normally distributed.
