...
We can conclude that the relation between *time* and *antisocial behavior* differs across children and we should account for it by allowing the random slope for *time*. *Read* does not have a varying slope and including it in the model, together with the other parameters, does not improve the fit of the model. 
...
We will inspect if each of the time invariant predictors, *momage* and *homecog*, moderates the relation between *time* and *anti*. Even though we have shown before that *mother's age* does not predict the differences in *antisocial behavior* at the first occasion across children, it is possible that it predicts the differences in the slope of *time*.
...
Comparing this model and the model without the predictors of slope for *time*, m5a, we see that the former does not fit significantly better, $\chi^2(3)=7.18, p >.05$, which is due to non-significant coefficients of *read* and *time X read*. Its AIC is, however, slightly lower, $AIC_{m5a}=3293.3, AIC_{m6a}=3292.1$
...
- with each one point increase in *cognitive stimulation*, the coefficient for *time* decreases by 0.05. This means that the differences in *antisocial behavior* across different measurement occasions are smaller for children who received more *cognitive stimulation* than for children with lower score on this variable.
