...
- create the model 2 with the fixed predictor *time*, which will be used as a baseline model to calculate $R^2$; **add some more about why this one is a baseline**
...
Both the intercept ($β_{00}=1.50, t(580) = 9.94, p <.001$) and the coefficient of *time* ($β_{time}=0.21, t(882) = 2.73, p < .01$) are again significant but their values are slightly higher. The intercept can be now interpreted as the predicted Behaviour Problems Index at the first measurement occasion for a person with average *reading recognition* score. The coefficient for *time* can be interpreted as follows: with each measurement occasion, the predicted *antisocial behavior* score goes up by .21 when *read* is held constant. 

We see that *reading recognition skills* is not significant ($\beta_{read} = -0.03, t(831) = -0.54, p > .05$).
...
- We observed a linear time trend where Behavior Problems Index increases at each measurement occasion. Furthermore, adding the *time* variable as a predictor improves the fit of the model.
